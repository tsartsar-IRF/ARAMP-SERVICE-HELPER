...
function requireRole(interaction, roleId) {
...
const commandsData = [
  new SlashCommandBuilder()
    .setName("xp")
    .setDescription("Check XP + progress (from Google Sheets)"),
...
function registerCommands(client) {
  ...
  client.commands.set("xp", {
    ...
    execute: async (interaction) => {
      ...
      if (!row) {
        return interaction.editReply(`No XP row found for **${nickname}** in XP tab (XP!A).`);
      }
  ...
  client.commands.set("log", {
    ...
    execute: async (interaction) => {
      // role lock
      if (!requireRole(interaction, LOG_ROLE_ID)) {
        return interaction.reply({ content: "❌ You don’t have permission to use **/log**.", ephemeral: true });
      }
      ...
      // safety check (even though dropdown already limits it)
      if (!ALLOWED_TYPES.includes(type)) {
        return interaction.editReply("❌ Invalid event type.");
      }

      // LOG tab columns: Time | Nickname | Type | Attendees | Proof
  ...
  client.commands.set("logselfpatrol", {
    ...
    execute: async (interaction) => {
      ...
      // SELF_PATROL tab columns: Time | Nickname | Start | End | Proof
